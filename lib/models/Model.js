class Model {
    ...
    static createNonCategorizedNode(identifier, name, type, properties) {
        if (name !== null) {
            return {identifier, name, type, properties};
        } else {
            return {identifier, name: 'Unknown Name', type, properties};
        }
    }
    ...
    setManyNodes(nodeList) {
        ...
        nodeList.forEach(node => {
            if (node.type && node.identifier) {
                ...
                if(properties){
                    categorizedNodes[type].push({identifier: node.identifier, name, properties});
                }else{
                    categorizedNodes[type].push({identifier: node.identifier, name});
                }
