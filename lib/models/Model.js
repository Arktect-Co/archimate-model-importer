class Model {
    ...
    static createNonCategorizedNode(identifier, name, type) {
        if (name !== null) {
            return {identifier, name, type};
        } else {
            return {identifier, name: 'Unknown Name', type};
        }
    }
    ...
    setManyNodes(nodeList) {
        ...
        nodeList.forEach(node => {
            if (node.type && node.identifier) {
                ...
                categorizedNodes[type].push({identifier: node.identifier, name: name});
