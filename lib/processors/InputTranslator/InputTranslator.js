...
class InputTranslator {
    ...
    constructor(inputInterpreter, outputModel, logger) {
    ...
    translate() {
        if (this.inputInterpreter.validate()) {
            ...
            if (!processViewDataEnable) {
                this.translateViews(nodes, relationships);

                this.log(`Sequential Views translated`);
            }

            organizationFolders.forEach((organizationFolder) => {
                this.processFolder(nodes, relationships,
                    organizationFolder, this.outputModel.model.landscape, processViewDataEnable);

                this.log(`Folders and nested views translated`);
            });
        } else { // Error - Isnt an Valid Archimate Model
        ...
        try {
            this.inputInterpreter.forEachModelRelationship((currentRelationship) => {
                ...
                if (this.inputInterpreter.isAssociationRelationship(currentRelationship)) {
                    let isdirected = this.inputInterpreter.getAssociationRelationshipIsDirected(currentRelationship);

                    if (!isdirected) { // This step is important to guarantee isBidirectional = undefined when isdirected = true
                    ...
                    isBidirectional = (direction.source && direction.target) || (!direction.source && !direction.target);

                    if (!isBidirectional) {
                        if (direction.source) { // Inverted direction
                ...
                relationships.push({
                    identifier: this.inputInterpreter.getRelationshipId(currentRelationship),
                    sourceId,
                    targetId,
     ...
     * @param {*} loadedNodes - Nodes already loaded with a plain array (without subobjects for types)
     * @param {*} loadedRelationships - Relationships already loaded with a plain array (without subobjects for types)
     ...
     */
    processFolder(loadedNodes, loadedRelationships, fileParentFolder, modelParentFolder, processViewDataEnable) {
        try {
            let currentModelFolder = this.outputModel.createFolder(uniqid(), this.inputInterpreter.getFolderName(fileParentFolder));
            let folders;
            let views;
            ...
            if (folders !== undefined) {
                folders.forEach((folder) => {
                    this.processFolder(loadedNodes, loadedRelationships, folder, currentModelFolder, processViewDataEnable);
                });
            }
            ...
            if (views !== undefined) {
                views.forEach((view) => {
                    this.processView(loadedNodes, loadedRelationships, currentModelFolder, view, processViewDataEnable);
                });
            }
    ...
    translateViews(loadedNodes, loadedRelationships) {
        this.inputInterpreter.forEachDiagram((currentDiagram) => {
            this.processView(loadedNodes, loadedRelationships, null, currentDiagram, true);
        });
    };
     ...
     */
    processView(loadedNodes, loadedRelationships, currentModelFolder, view, processViewDataEnable) {
        try {
            let nodes = [];
            let relationships = [];
            ...
            if (processViewDataEnable) {
                ...
                this.processViewElements(view, loadedNodes, loadedRelationships, nodes, relationships, viewElements, bounds, null);

                if (!this.inputInterpreter.hasViewElementChildRelationships) { // Then process relationships
                ...
                this.outputModel.addView(viewId, viewName, bounds, nodes, relationships);
            }

            this.outputModel.addFolderView(currentModelFolder, viewId, viewName);
        } catch (e) {
     ...
     */
    processViewElements(view, loadedNodes, loadedRelationships, nodesResult, relationshipsResult, viewElements, bounds, parent, parentViewElements) {
        if (viewElements !== undefined) {
            viewElements.forEach((viewElement) => {
                let viewNodeId = this.inputInterpreter.getViewElementViewId(viewElement);
                let positionX = this.inputInterpreter.getViewElementPositionX(viewElement, parent, parentViewElements) || 0;
                let positionY = this.inputInterpreter.getViewElementPositionY(viewElement, parent, parentViewElements) || 0;
                ...
                if (this.inputInterpreter.isViewObject(viewElement)) {
                    ...
                    let el = _.find(loadedNodes, (e) => {
                        return e.identifier.localeCompare(modelElementId) === 0;
                    });

                    if (el !== undefined) {
                        elementName = el.name;
                        elementType = el.type.toLowerCase();
                    } else { // Else it is a group
                        elementType = "group";
                    }
                    ...
                    if (this.inputInterpreter.hasViewElementChildRelationships) {
                        let sourceConnections = this.inputInterpreter.getViewElementSourceRelationships(viewElement);
                ...
                if (nestedElements !== undefined && nestedElements.length > 0) {
     ...
     */
    processViewRelationship(modelSourceNodeId, viewNodes, loadedRelationships, relationshipsResult, viewRelationship) {
        if (viewRelationship !== undefined) {
            ...
            let isBidirectional = undefined;
            let sourceViewElementId = this.inputInterpreter.getViewRelationshipSourceElementId(viewRelationship);
            let targetViewElementId = this.inputInterpreter.getViewRelationshipTargetElementId(viewRelationship);
            ...
            if (sourceElement !== undefined && targetElement !== undefined) { // Verifying orphan relationship
                relationshipBendpoints = this.inputInterpreter.getViewRelationshipBendpoints(viewRelationship);
                modelRelationshipId = this.inputInterpreter.getViewRelationshipModelId(viewRelationship);

                if (relationshipBendpoints !== undefined) {
                    ...
                    for (let j = 0; j < len; j++) {
                        ...
                        // TODO: Checking bounds limits for bendpoints

                        let resultedBendpoint = this.inputInterpreter.getViewRelationshipBendpoint(bendpoint, j, len, sourceElement, targetElement, viewNodes) || {
                            x: 0,
                            y: 0
                        };
                ...
                let rel = _.find(loadedRelationships, (r) => {
                    return r.identifier.localeCompare(modelRelationshipId) === 0;
                });
                ...
                viewModelRelationship = Model.createViewRelationship(
                    ...
                    sourceViewElementId,
                    targetViewElementId,
                    this.inputInterpreter.getViewRelationshipId(viewRelationship),
                    relationshipType,
                    bendpoints,
                    isBidirectional
                );
