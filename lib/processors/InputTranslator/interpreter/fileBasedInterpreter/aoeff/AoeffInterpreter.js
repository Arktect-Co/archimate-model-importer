...
class AoeffInterpreter {
    ...
    getNodeDocumentation(node) {
        return node.documentation && node.documentation[0] && node.documentation[0]._ ? node.documentation[0]._ : null;
    }
    ...
    getViewName(view) {
        if (view !== undefined && view.name !== undefined) { // It's a view of the Views folder
            return view.name[0]._ ? view.name[0]._ : view.name[0];
        } else { // It's a view of the Organizations' folder
    ...
    getOrganizationFolders() {
        ...
        if (Array.isArray(this.model.organizations) && this.model.organizations[0] && this.model.organizations[0].item) {
    ...
    validate() {
        return Array.isArray(this.model.elements) &&
